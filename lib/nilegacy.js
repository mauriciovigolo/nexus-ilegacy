...
function loadProcessorOptions(args) {
  ...
  var repo = args.shift();
  if (repo !== 'maven' && repo !== 'npm' && repo !== 'nuget') {
    printError('Incorrect repository option: %s', 'Valid repo keys: maven | npm | nuget', repo);
  }
  ...
  while (args.length) {
    ...
    switch (arg) {
      case '--repoUrl':
        repoUrl = shiftArg(args);
        break;
      case '--repositoryId':
        repositoryId = shiftArg(args);
        break;
      case '--generatePom':
        generatePom = true;
        break;
      case '--apiKey':
        apiKey = shiftArg(args);
        break;
      case '--groupId':
        groupId = shiftArg(args);
        break;
      case '--examples':
      ...
      case '--help':
        ...
        break;
      case '--version':
        printError('Invalid syntax. For version info type: nilegacy --version');
        break;
      case '--output':
        outputLogFile = shiftArg(args);
  ...
  } else if ((repo === 'maven') && !repositoryId) {
...
function searchFiles() {
  ...
  if (repoOptions.repo === 'maven') {
    fileNamePattern = '**/**.jar';
  } else if (repoOptions.repo === 'npm') {
    fileNamePattern = ['**/package.json', '**/**.tgz', '!node_modules'];
...
function help() {
  ...
  print('');
  print('Repo:');
  print('%s\tspecify the type of repo (maven|npm|nuget)', info(lPad('repo', 18)));
  print('');
  print('Options:');
